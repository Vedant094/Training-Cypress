...
SELECT * FROM customers WHERE contactlastname IN ('taylor','brown');
...
SELECT * FROM customers WHERE phone IS NULL;-- not considered as NULL
...
SELECT ordernumber,orderlinenumber,quantityordered,priceeach,quantityordered*priceeach
FROM orderdetails;
...
SELECT ordernumber,orderlinenumber,quantityordered*priceeach AS TotalPrice FROM orderdetails
...
SELECT jobtitle,COUNT(*) EmpNO FROM employees GROUP BY jobtitle;
...
SELECT UPPER(CONCAT(contactfirstname,' ',contactlastname)) AS 'Full Name', REVERSE(country), REPLACE(state,'NY','KA')
FROM customers;
